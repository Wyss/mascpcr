...
# import mascpcr.indexutils as indexutils
import indexutils
...
def test():
    ...
    GENOME = os.path.join(LOCAL_DIR, '2014_02_18_gen9_54_failed_seg_fixed_with_gc_fixes.fa')
    ...
    try:
        idx_lut = np.load(os.path.join(LOCAL_DIR, 'recoli_index_lut.npy'))
    except IOError:
        idx_lut = buildIndex(GENOME, REF_GENOME, fill_gaps=False,
                             smooth_edges=False)
        fillGaps(idx_lut)
        smoothEdges(idx_lut)
        np.save(os.path.join(LOCAL_DIR, 'recoli_index_lut'), idx_lut)
