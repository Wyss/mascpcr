...
def getPrimerPairProductSize(pc1, pc2):
    ...
    Args:
        ``pc1``: PrimerCandidate namedtuple -- one primer in a pair
        ``pc2``: PrimerCandidate namedtuple -- the other primer in a pair
    ...
    if pc1.strand == pc2.strand:
        raise ValueError('Both PrimerCandidates bind to the same strand')
    pc1_5p_idx = pc1.idx
    pc2_5p_idx = pc2.idx
    if pc1.strand == -1:
...
def writePrimerRec(fd, pc, idx_lut, recoded_idx=True, gen_wt_idx=False):
    ...
    Args:
        ``fd``: open file descriptor to which to write the primer record
        ``pc``: PrimerCandidate namedtuple for which to write the record

    Kwargs:
        ``recoded_idx``: (bool) whether or not the primer's index is in the 
                         recoded genome idx space or the wt genome idx space
        ``gen_wt_idx``: if ``recoded_idx`` is ``True``, specifies that the
                        index on the wt genome should be calculated (used for 
                        common primers)
...
def writeCsvReport(idx_lut, set_of_primer_sets, combined_bins, params):
    ...
    Args:
        ``idx_lut``: numpy array of index mapping from recoded to wt genome
        ``set_of_primer_sets``: list of length equal to the number of bins
                                specifying the primer set chosen from each
                                bin (see code in :module:``mascpcr.pipeline``)
        ``combined_bins``: list of length equal to the number of bins 
                           containing sorted lists of primer sets for each
                           respective bin
